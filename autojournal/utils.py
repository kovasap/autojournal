...
def retry_on_error(
    function, errors=(ConnectionResetError, ), sleep_time_secs=30,
    num_retries=5):
  last_error = None
  for retry in range(num_retries):
    try:
      ...
      last_error = e
      print(
          f'Hit {e}, sleeping for {sleep_time_secs}s, then trying again.. '
          f'(attempt {retry}/{num_retries})')
...
def timestamp_ms_to_event_time(
    timestamp_ms: int,
...
def split_on_gaps(values, threshold, key=lambda o: o, last_key=None):
  ...
  if split_points:
    ...
    return [
        values[split_points[i]:split_points[i + 1]]
